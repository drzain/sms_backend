      ...
      <div class="main-sidebar sidebar-style-2">
        <aside id="sidebar-wrapper">
          <div class="sidebar-brand">
            <a href="<?php echo base_url(); ?>home">Beyond SMS</a>
          </div>
          <div class="sidebar-brand sidebar-brand-sm">
            <a href="<?php echo base_url(); ?>home">Yd</a>
          </div>
          <?php 
            $ci =&get_instance();
            $ci->load->model('m_menu');
            $getmenuparent = $ci->m_menu->getmenuparent($this->session->userdata('role_id'));
            $uri = $this->uri->segment(1);
            $uri2 = $this->uri->segment(1);
            $data['menu'] = null;
            foreach ($getmenuparent->result() as $resmenuparent) {
              if ($resmenuparent->link == ''){
                if($uri == $resmenuparent->link){$status = 'active';}else{$status = '';}
              $data['menu'] = $data['menu'].'<li class="dropdown '.$status.'">
                <a href="'.site_url().''.$resmenuparent->link.'" class="nav-link has-dropdown"><i class="'.$resmenuparent->style.'"></i><span>'.$resmenuparent->caption.'</span></a>
                <ul class="dropdown-menu">';

              $data['menu2'] = null;
             $getmenuchild = $ci->m_menu->getmenuchild($resmenuparent->id,$this->session->userdata('role_id'));
             foreach ($getmenuchild->result() as $resmenuchild) {
              if($uri2 == $resmenuchild->link){$status2 = 'active';}else{$status2 = '';}
              $data['menu2']=$data['menu2'].'<li class="'.$status2.'"><a class="nav-link" href="'.site_url().''.$resmenuchild->link.'">'.$resmenuchild->caption.'</a></li>';
              }
              $data['menu'] = $data['menu'].$data['menu2'].'</ul></li>';       
            }
            else{
              if($uri == $resmenuparent->link){$status = 'active';}else{$status = '';}
              $data['menu']=  $data['menu'].'<li class= "'.$status.'"><a class="nav-link" href="'.site_url().''.$resmenuparent->link.'"><i class="'.$resmenuparent->style.'"></i> <span>'.$resmenuparent->caption.'</span></a></li>';
            }
          }
          ?>
